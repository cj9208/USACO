...
struct B{
	...
	{
		int len = 0;
		for(int i=12;i>=0;i--) {  //dilete the added 1 
			if(bs[i]) {
				len = i;
				break;
			}
...
bool cmp(const B &a,const B &b) //frequency : from big to small, string : from small to big
{         
	if(a.t!=b.t)
		return a.t > b.t;
...
int main()
{
	...
	for(int i=0;i!=seq.size();i++)
		for(int j=a;j<=b;j++) 
			if(i+j-1 < seq.size()){
				...
				bs[j]=1; //add 1 to differentiate 0, 00
				t[bs.to_ulong()]++;
			}
	...
	vb.push_back(B(bitset<BN>(0),-1));	//end flag
	int p = 0,last = -1;
	while(n--)	{
		if( vb[p].t == -1) 
			break;
			
		if( vb[p].t != last){ //string with new frequency
